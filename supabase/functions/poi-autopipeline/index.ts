        ...
        function: {
          ...
          parameters: {
            ...
            properties: {
              ...
              opening_hours: { type: "object", description: "Horaires d'ouverture structurés par jour" },
              must_see_details: { type: "string", description: "Ce qu'il faut absolument voir à cet endroit" },
              must_try: { type: "string", description: "Ce qu'il faut essayer (plat, activité, expérience)" },
              must_visit_nearby: { type: "string", description: "Lieux proches à ne pas manquer" },
              ...
              ruelle_etroite: { type: "boolean", description: "Le POI est-il dans une ruelle étroite ?" },
            },
      ...
      for (const poi of batch ?? []) {
        try {
          ...
          await supabase.from("medina_pois").update({
            ...
            must_see_details: r.must_see_details ?? null,
            must_try: r.must_try ?? null,
            must_visit_nearby: r.must_visit_nearby ?? null,
            is_photo_spot: r.is_photo_spot ?? false,
            photo_tip: r.photo_tip ?? null,
            ruelle_etroite: r.ruelle_etroite ?? false,
